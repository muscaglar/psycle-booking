...
| 7 | Stored data is untrusted: coerced where it is read, escaped where it is printed | `pure:stored-data` (`_cleanStoredId`, `_cleanStoredIdList`, `_cleanStoredNumbers`, `_cleanStoredHistory`, `_cleanStoredBikePrefs`); `_planSettingsImport` in `js/settings.js` | `tests/suites/import-validate.js` |
