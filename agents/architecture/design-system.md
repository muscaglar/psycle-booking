...
| **Favourite star** | `--fav` — the one colour of a starred instructor's ★ (`.favs-star.is-fav`, `.fav-star.fav-on`). Set per theme and held to ≥3:1 (a graphic) on that theme's ground, surface, wells and sunken: Cloud wears the class palette's deep "sun", the dark themes a gold, Handheld its heading lime (tests/suites/10b-themes.js) | `color: var(--fav)` |
