...
| `CapacitorCalendar` | `checkAllPermissions`, `requestAllPermissions`, `listCalendars`, `listEventsInRange`, `deleteEventsById`, `deleteEvent`, `createEvent` | **Three call shapes change — step 6.** `requestAllPermissions` has been marked deprecated since 7.1.0 and still ships in 8.6.0. It asks for full Reminders access as well, exactly as 6.7.2 did (same code path), so nothing new — 8.x also offers `requestFullCalendarAccess()` if you ever want to stop asking for Reminders. A behaviour change; not part of this upgrade. |
