...
7. **Never edit unit.js's built-in checks or js/app.js's `waitlist:pure` block to make a test pass** → they pin `escapeHTML`, `PsycleAPI.categorizeError`, `PsycleDiag`'s record and contract check, the London resolver and the waitlist rules → unit.js runs that block itself. A built-in check goes only together with the code it tests, and only when the owner says so ([decisions.md](../agents/decisions.md) section 7).
...
| The Android app | `18-android.js` (Back's decision and actor — and the shipped `bookClass` it sits over — Android copy, the bridge booted as `'android'`, a forged notification tap, and the iPhone path held to a digest of its plugin calls) · `19-android-project.js` (reads ios-app/android/: the manifest and both backup rule files, MainActivity, signing material, every res/ XML and reference, the CI jobs, the widget's Java and layouts — its one-line styles, its plan's constants against the XML's numbers, the alarm's `finally` — and the class countdown's: a pure planner, ONE channel at IMPORTANCE_LOW, the builder calls and their `SDK_INT` tests, the two unexported receivers (the planning one hears four system broadcasts and reads nothing from an intent; a swipe's delete intent reaches the other), every alarm a ten-minute `setWindow` on `RTC` and no bare `AlarmManager.set(`, no new permission, ONE tap builder, and the debug hook `CountdownProofReceiver` held to ci.yml's three broadcasts (component, string extras, tag), whose verdict patterns — the timeout's among them — it runs against a record written the way the platform prints one; it cannot compile anything) · `20-android-widget.js` (the bridge booted as `'android'` WITH the three plugin twins, on 18's `launch`: what the widget's store receives, a retained and a hostile tap, sign-out against expiry, the twins' names against the Java, the JVM tests' copy of the class-colour palette against js/theme.js) · `21-android-countdown.js` (the countdown's bridge half, on the same launch: `countdown_enabled` on every Android pass — after the three keys, before the ONE reload — '0' with Class reminders off, a flip that goes out by itself when no pass carries it, an older app served without a word, NEVER on the iPhone, and the Java's allow-list held to the bridge's literal and its two values). The widget's and the countdown's own rules are JVM tests under ios-app/android/app/src/test/, which only CI runs |
