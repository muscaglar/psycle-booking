...
| **Offline-queue item** | `psycle_offline_queue` (`js/reliability.js`) | booking `{qid, owner, eventId, slots, spaces, heldAtQueue, timestamp, startAt?, label?}` · cancel `{qid, owner, type: 'cancel', eventId, bookingIds, timestamp, startAt?, label?}` | Stamped by `_stampQueueItem`. Verdicts in §3.4 |
