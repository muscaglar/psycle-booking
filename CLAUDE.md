...
│   ├── app.js              # Core: API, session, search + window cache, booking, waitlist, My Bookings, class detail sheet
...
│   ├── tabs.js             # 4-tab navigation, insights rendering, share card, Membership, "Your usual week" card + confirm sheet
...
│   ├── redesign.css        # "Psync redesign" skin: top bar, Discover cards, pill rows, sticky day headers (layout + type only; colours from tokens)
...
| Tab | Name | Contents |
|-----|------|----------|
| 1 | **Discover** | Filters shown inline, live (no Search button): date row — calendar button first, then Today · Tomorrow · 7 days · Next week · 14 days; Time row — Before 9 · 9–5 · After 5 + "Available only" (filters the loaded timetable with no network call; never hides a class you hold or are waitlisted on); studio multi-select chips; class-type pills (+ Strength / Reformer sub-pills); instructor box with ★ Favs and an S/A quick filter (appears once an instructor is ranked S or A) → "Updated … · Refresh" → results (the empty state offers Quick picks and Recent searches pills; error and empty states carry actions): cards with an availability line ("Only 2 left" / "25 spots left" / "Waitlist open" / "Fully booked"; counts only while the timetable is under 30 minutes old), day headers that stick on phones and read Today / Tomorrow. Pull-to-refresh refreshes the window |
...
| 3 | **Stats** | History-sync banner, quick stats, habit alerts, streaks & milestones, lapsed favourites, recommendations, instructor suggestions (You might like / New to you), class types, instructor map, variety, heatmap, year in review, share card. "This month", "All time" and the share image count only classes that have started. Signed out: the Sign-in hero only |
| 4 | **Membership** | Cost tracker, plan info, Appearance (theme picker), Settings rows (Bike / spot preferences · Reminders and Calendar sync on iOS only · Data & privacy), Sign out (asks first), instructor rankings & favourites, about/affiliation. (Settings panel = app-focused: reminders, bike prefs, calendar sync, export/import, bug report + diagnostics) |
...
| Add a new Stats / Membership section  | js/tabs.js               |
...
### History Sync Flow
```
First login → welcome modal (dismissal is remembered) → "Sync my history" → fetches /bookings?type=previous
