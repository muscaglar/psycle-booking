...
└── ios-app/                # Capacitor wrapper for BOTH native apps (the name is historical): one www/, one bridge, two native projects
    ...
    │       ├── test/java/…/widget/                     # JVM unit tests (JUnit 4) of the two pure classes — CI runs them before the APK
    │       └── debug/                                  # WidgetPreviewActivity + its manifest: DEBUG builds only, what CI's emulator photographs
...
| Release the Android app, or edit its store copy | ios-app/ANDROID.md · ios-app/PLAY_STORE_LISTING.md |
