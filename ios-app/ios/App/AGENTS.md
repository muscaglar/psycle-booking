...
12. **App/TipJarPlugin.swift is the ONLY StoreKit code, and a tip unlocks nothing**: three consumable ids in `PsycleTipProducts.ids` (the same list as js/tabs.js and the Android twin), only a VERIFIED transaction is answered `purchased`, and it is finished first; no receipt leaves the device. A new Swift file in the app target needs four project.pbxproj entries (build file, file reference, group, Sources), as this one has → 24-tip-jar.js; the simulator build proves the wiring.
