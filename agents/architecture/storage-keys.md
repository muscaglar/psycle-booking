...
| ~~psycle_instructor_tiers~~ |  | RETIRED 2026-09-22 — instructor grades were removed with no migration (decisions.md section 3: a star is the only mark on a person). Deleted at every launch: `RETIRED_STORAGE_KEYS` (js/app.js) on the web, `RETIRED_KEYS` (native-bridge.js, `Preferences.remove`) in the apps; out of export, import, the account stash and `SYNC_KEYS`. An old backup's grades are written nowhere (import-validate.js) |
