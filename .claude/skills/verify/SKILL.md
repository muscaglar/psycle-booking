...
- Allocation announcement: `psycle_waitlist_places` remembers held places; make an event move from `/waitlists` to `/bookings` between two `fetchMyBookings()` calls → `confirmModal` titled "You're in — Psycle gave you a spot", badge "From waitlist", `waitlist:allocated` in the action log. `clearToken()` must remove the key.
