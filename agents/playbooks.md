...
- **Read this when** your task matches a heading below: read P0 and that one playbook — never the whole file (≈ 12,200 tokens).
...
| P14 | Change the Android native project — and prove it | 1,450 |
...
Sources: ios-app/android/ — `MainActivity.java`, the three plugin twins beside it, `widget/` (the home-screen widget), `AndroidManifest.xml`, `res/`, `app/build.gradle`, `variables.gradle`; `app/src/test/` (JVM tests) and `app/src/debug/` (the widget preview). What DIFFERS by platform is mostly not there: it is an `IS_ANDROID` branch of ios-app/www/native-bridge.js, or a `getPlatform() === 'android'` test in js/ (step 4). The widget is the exception — and even it is driven by the bridge's existing, branch-free path (G11).
...
3. `npm run ci` — 19-android-project.js reads the project (the manifest, MainActivity's Back contract, signing material, every XML, every reference, the names the bridge and the project share, and the widget: RemoteViews-safe layouts, immutable PendingIntents, an `SDK_INT` test before an API above 22, the twins registered before `super.onCreate`, no `android.*` in the pure class, the preview under app/src/debug/ only). When what you changed CAN be read — a permission, a resource the bridge names, a Gradle value — add the assertion there. Never loosen one to get past it.
...
5. **Prove the compile**: push to an `android/**` branch (never `main` unasked — hard rule 6) and read the `android-build` job: `npm ci` → `npm run sync:android` → `./gradlew :app:testDebugUnitTest` (the JVM tests of app/src/test/ — BLOCKING; a red one prints its expected / actual from the XML result in the step "Show why a JVM test failed", and the report is the artifact `android-unit-test-report`) → `./gradlew assembleDebug` → the artifact `psync-debug-apk`; `:app:lintDebug` is advisory (artifact `android-lint-report`). The advisory `android-smoke` job then installs that APK on an emulator, launches it, sends ONE Back key, opens the debug-only `WidgetPreviewActivity` nine times and uploads `android-smoke` (launch.png, after-back.png, pid-after-back.txt, logcat.txt, widget-compact-light / -wide-light / -wide-dark / -compact-off / -empty / -compact-four-seats / -compact-narrow / -compact-smallest / -compact-large-type.png, logcat-widget.txt): a person reads those — LOOK at the nine pictures (is the seat badge whole? does anything end mid-word?), nothing judges them beyond a `PsyncWidgetPreview` error in the log, which is a widget `RemoteViews.apply()` refused; it never taps. Reading a commit's checks without signing in: the `curl … /check-runs` form in ios-app/CICD.md → "Notes / gotchas", selecting by the job's name. With a toolchain (JDK 17, Android SDK Platform 35): `cd ios-app && npm ci && npm run android:test && npm run android:debug`.
6. **A red compile**: read the FIRST error of the Gradle log (aapt2 and the manifest merger stop at the first one), fix that one thing, push again. `--stacktrace` is already on. A red JVM test: the expectations have passed only against a stand-in for org.json, off CI, so decide from the printed expected / actual whether the CODE, the TEST or the real parser's leniency is what differs before touching anything.
...
10. **A release is the owner's**: the keystore, `bundleRelease`, Play Console — ios-app/ANDROID.md → "Release". No key, password or keystore.properties is ever committed; a Play submission is blocked on the target API level until the Capacitor upgrade (`agents/backlog.md`).
