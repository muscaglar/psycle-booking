...
| **Builds cost: push to `main` once per piece of work** (2026-09-23). The owner, after a day of six pushes: "can we optimise the Github runner actions as its costing alot". Work on a branch, prove it with `npm run ci` and the browser check, push once; push a lone fix at once only when the owner is waiting to see it on a phone. CI itself starts on `main`, `android/**` and pull requests only, cancels a superseded run, and skips a native build a push cannot have changed. | The repository is public, so GitHub bills none of it; the metered build is Xcode Cloud's archive, one per push to `main` (25 hours a month). | .github/workflows/ci.yml, .github/scripts/changed-areas.js, tests/suites/25-ci-cost.js; ios-app/CICD.md "What a push to `main` costs" |
| An Xcode Cloud start condition that builds only when `ios-app/` changes: DECLINED (2026-09-23). The owner: "seems pointless". CLOSED. | Nearly every change to the app changes `ios-app/www/`: it would have saved 5 of 52 archives. | ios-app/CICD.md |
| **Code with no caller goes, and so do the tests that only test it** (2026-09-23). Asked whether the rest of an unused API layer (`validate`, `field`, `parseJson` of js/api-client.js and the missing-field chain of js/diagnostic.js: no caller in the app, but tested by tests/unit.js's built-in checks, which tests/AGENTS.md says are never edited to make a test pass) should stay, the owner: "Remove them." That rule still stands for every OTHER built-in check: it forbids weakening a guard, not deleting the test of deleted code. | Dead code is read, indexed and maintained like live code; two of the removed paths could send a `POST /bookings` with no confirmation. Each removal was proved unreachable by two independent checks — from Swift and Java too — before it was cut. | tests/unit.js; tests/AGENTS.md rule 7; agents/HANDOVER.md, the 2026-09-23 entry |
